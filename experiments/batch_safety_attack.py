...
def _resolve_audio_path(manifest_path: Path, value: Any) -> Path:
    if value is None or not str(value).strip():
        raise ValueError("clean_audio_path is required for every attack case")
    path = Path(str(value).strip()).expanduser()
...
def run_batch(
    ...
    *,
    ...
    for index, (row, case_id) in enumerate(zip(records, case_ids)):
        ...
        input_audio = _artifact_identity(
            row.get("clean_audio_path"), base=manifest.parent
        )
    ...
    for (
        ...
        try:
            ...
            audio_path = _resolve_audio_path(manifest, row.get("clean_audio_path"))
            case_reference = None
            if method in INTERNAL_LAYER_METHODS:
